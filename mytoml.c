 ...
 *
 * @param[in,out]  filename  A toml filename.
 * @returns the `tokenizer` object.
 */
tokenizer_t *_Mytoml_NewTokenizer(char *filename);
...
tokenizer_t *_Mytoml_NewTokenizer(char *input)
{
...
MYTOML_API TomlKey *tomlLoad(char *file)
{
    ...
    tokenizer_t *tok = _Mytoml_NewTokenizer(file);
...
MYTOML_API void toml_key_dump(TomlKey *k)
{
...
MYTOML_API void toml_value_dump(TomlValue *v)
{
    switch (v->type)
    {
    case TOML_STRING:
